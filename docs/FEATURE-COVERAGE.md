...
- **Névjegy page** (purple AppBar "Névjegy", on the same grey as the Templomok and Misék lists), under a floating **Visszajelzés** button (bottom centre, in view without scrolling) that opens the feedback mail; top to bottom: the **Mai templom ajánlatunk** card; **Fejlesztő** — Szent József Hackathon, with the version ("Verzió: x.y.z (build)"); **Forráskód** — a link to the GitHub repository; and a **miserend.hu** tile that opens the web version in the browser.
