...
**Files:** `lib/widgets/feedback_mail.dart`, `lib/about/about_page.dart`, `lib/about/church_of_the_day_loader.dart`; spec 0009, spec 0014
...
- **Névjegy page** (purple AppBar "Névjegy", on the same grey as the Templomok and Misék lists), under a floating **Visszajelzés** button (bottom centre, in view without scrolling) that opens the feedback mail; top to bottom: the **Mai templom ajánlatunk** card; **Kiadó** — Jézus Társasága Magyarországi Rendtartománya, 1085 Budapest, Horánszky u. 20., link to jezsuita.hu, and the 1% offer to the Jézus Társasága Alapítvány with its tax number and a copy button; **Fejlesztő** — Szent József Hackathon, with the version ("Verzió: x.y.z (build)"); **Forráskód** — a link to the GitHub repository; and a **miserend.hu** tile that opens the web version in the browser.
